...
export default function HocSession() {
  ...
  useEffect(() => {
    ...
    socket.on('student_signed', onStudentSigned);
    socket.on('session_closed', onSessionClosed);

    return () => {
      socket.emit('leave_session', session_id);
      socket.off('student_signed', onStudentSigned);
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-1">
              {session?.course_code} · {session?.class_type} class
            </p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={handleClose}
              disabled={closing}
              className="bg-red-600 hover:bg-red-700 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-semibold transition"
            >
